...
def test_fetch_headlines_stores_none_for_missing_url(monkeypatch):
    payload = {
        "articles": [
            {
                "source": {"name": "News Source"},
    ...
    assert cleaned == ["No URL headline"]
    stored_records = mock_write.call_args.args[0]
    assert stored_records[0]["url"] is None
...
def test_build_news_scheduler_registers_five_minute_job(monkeypatch):
    ...
    monkeypatch.setattr(news_fetcher, "init_db", mock_init_db)
